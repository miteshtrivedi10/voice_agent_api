...
EMBEDDING_API_URL = os.getenv("EMBEDDING_API_URL", "http://localhost:8001/generate-embedding")
...
async def insert_file_details_async(file_data: FileDetails):
    ...
    try:
        ...
        if success:
            ...
            absolute_filepath = f"uploaded_files/{file_data.file_name}"
            embedding_response = await generate_embedding(file_data, absolute_filepath)
            
            if embedding_response and embedding_response.status == "success":
                logger.info(f"Embedding generation successful for user_id: {file_data.user_id}, file_id: {file_data.file_id}")
                
                # Process the embedding response and store Q&A pairs
                await process_embedding_response(file_data, embedding_response)
            elif embedding_response:
                logger.warning(f"Embedding generation failed with status: {embedding_response.status} for user_id: {file_data.user_id}, file_id: {file_data.file_id}")
            else:
                logger.error(f"Embedding generation failed for user_id: {file_data.user_id}, file_id: {file_data.file_id}")
        else:
...
async def generate_embedding(file_data: FileDetails, absolute_filepath: str) -> Optional[GenerateEmbeddingResponse]:
    """
    ...
    try:
        ...
        logger.info(f"Calling embedding API for user_id: {file_data.user_id}, file_id: {file_data.file_id}")
        
        # Create HTTP client with timeout of 3 minutes (180 seconds)
        async with httpx.AsyncClient(timeout=180.0) as client:
            response = await client.post(EMBEDDING_API_URL, json=request_data.model_dump())
        ...
        if response.status_code == 200:
            ...
            logger.info(f"Embedding API successful for user_id: {file_data.user_id}, file_id: {file_data.file_id}")
            return embedding_response
        else:
            logger.error(f"Embedding API returned status code {response.status_code} for user_id: {file_data.user_id}, file_id: {file_data.file_id}")
            return None
            
    except asyncio.TimeoutError:
        logger.error(f"Embedding API timeout (3 minutes) for user_id: {file_data.user_id}, file_id: {file_data.file_id}")
        return None
    except Exception as e:
        logger.error(f"Error calling embedding API for user_id: {file_data.user_id}, file_id: {file_data.file_id}: {e}")
        return None


async def process_embedding_response(file_data: FileDetails, embedding_response: GenerateEmbeddingResponse):
    """
    ...
    try:
        ...
        for i, qna_pair in enumerate(embedding_response.question_and_answers):
            ...
            if not success:
                logger.error(f"Failed to insert Q&A pair {i} for user_id: {file_data.user_id}, file_id: {file_data.file_id}")
        
        logger.info(f"Successfully processed {len(embedding_response.question_and_answers)} Q&A pairs for user_id: {file_data.user_id}, file_id: {file_data.file_id}")
        
    except Exception as e:
        logger.error(f"Error processing embedding response for user_id: {file_data.user_id}, file_id: {file_data.file_id}: {e}")
